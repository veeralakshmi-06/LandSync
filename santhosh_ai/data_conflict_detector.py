...
class DataConflictDetector:
    ...
    def parse_area_value(cls, area_input: Any) -> Tuple[Optional[float], str]:
        """
        Parses string or numeric area input (e.g. '1.25 acre', '54450 sq_ft', 1.25).
        ...
        text = str(area_input).strip().lower()
    ...
    def detect_conflicts(self, records: List[Dict[str, Any]], primary_record_id: Optional[str] = None) -> List[ConflictItem]:
        ...
        belonging to the same land parcel.
        """
        conflicts: List[ConflictItem] = []
        if not records:
            return conflicts

        # If only 1 record, check for internal missing values
        if len(records) == 1:
            conflicts.extend(self._check_missing_values(records[0]))
            return conflicts

        # Baseline record vs other matched source records
        base = records[0]
        base_src = base.get("source", "Primary Record")

        # 1. Missing values check across all records
        for rec in records:
            conflicts.extend(self._check_missing_values(rec))

        # 2. Compare baseline with secondary records
        for rec in records[1:]:
            rec_src = rec.get("source", "Secondary Record")
            ...
            if base_area_acres is not None and comp_area_acres is not None:
                ...
                pct_diff = (diff / max(base_area_acres, 0.0001)) * 100.0
    ...
    def _check_missing_values(self, record: Dict[str, Any]) -> List[ConflictItem]:
        ...
        for field_name, severity in mandatory_fields:
            ...
            if val is None or str(val).strip() == "" or str(val).strip().lower() in ["null", "none", "nan", "na"]:
                conflicts.append(ConflictItem(
